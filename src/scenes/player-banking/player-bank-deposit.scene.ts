...
@Scene()
export class PlayerBankDepositScene {
  constructor(
  ...
  async onSceneEnter(
    ...
    if (businesses.length === 0) {
      ...
      return await ctx.reply(
        'Не найдены бизнесы на которые можно перевести средства.',
      );
    }

    ctx.scene.session = {
      menuMsg: await ctx.replyMenu(
        'Выберите бизнес на которой Вы бы хотели перевести средства:',
        [
  ...
  async onAmountValidation(
    ...
    try {
      ...
      return ctx.reply(
        `Уважаемый клиент! Извините, но банк не принимает '${ctx.msg.text}' для перевода средств. Он предпочитает обычные цифры, которые можно посчитать! Пожалуйста, введите настоящее число. С уважением, ваш Любящий Цифры Банк!`,
      );
    }

    if (amount.isNaN()) {
      return ctx.reply(
        `Уважаемый клиент! Мы искренне сожалеем, что Вы столкнулись с математической аномалией, пытаясь использовать 'NaN' как сумму для перевода средств. В нашем банке мы привыкли к цифрам, которые можно посчитать на пальцах. Предлагаем Вам выбрать более традиционное число. С уважением, ваш Банк Реальных Чисел`,
      );
    }
    if (!amount.isFinite()) {
      return ctx.reply(
    ...
    if (amount.isZero()) {
      return ctx.reply(
        `Уважаемый клиент! Похоже, Вы запросили перевод на сумму равную 0. Наш банк немного растерялся, так как обычно люди приходят за деньгами, а не за пустотой! Пожалуйста, введите число больше нуля, чтобы мы могли вам помочь. С уважением, ваш Ненулевой Банк!`,
      );
    }
    ...
    if (playerAccount.balance.minus(amount).isNegative()) {
      return ctx.reply(
        `Уважаемый клиент! Похоже, что желаемая сумма к переводу превышает ваш баланс. Наш банк умеет многое, но волшебства – не его сильная сторона! Пожалуйста, попробуйте сумму поменьше. С уважением, ваш Реалистичный Банк!`,
      );
    }

    try {
      await this.businessBankingEntity.deposit(
        playerAccount.id,
        businessAccount.id,
        amount,
      );
